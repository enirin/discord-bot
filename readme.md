...
また、ゲームサーバー管理系の機能は Discord コマンドだけでなく、自然言語での依頼からも処理できるスキル構成にしています。
...
現在はこの API ファースト設計の上に、Discord コマンドと自然言語入力の両方から同じサーバー操作スキルを呼び出せるようにしています。たとえば以下のような依頼に対応できます。
...
また、Discord へは何も送らずにゲームサーバー一覧キャッシュを扱いたい場合は、`/catalog/game-servers` を利用できます。`POST /catalog/game-servers` は Bot 内部の catalog repository に対して `/list` の再取得を指示し、`GET /catalog/game-servers` は再取得せず現在のキャッシュ内容だけを返します。
...
これらの処理は内部的にはスキルとして整理されているため、同じチャンネルで自然言語でも呼び出せます。
...
自然言語からゲームサーバーを特定する別名は、ゲーム管理 API の `/list` で返る各サーバー情報の `server_aliases` を利用します。Bot 側は `servers` 一覧をキャッシュし、キャッシュ未作成時だけ `/list` を取得します。
