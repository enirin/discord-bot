...
また、ゲームサーバー管理系の機能は Discord コマンドだけでなく、自然言語での依頼からも処理できるスキル構成にしています。現在のゲーム管理連携は REST 直結ではなく、Game-Server-Management-API が公開する MCP server を bot 側が host として利用する構成です。
内部構成としては、Discord 固有処理、スキル層、アプリケーションサービス層、インフラ層を分ける形に整理しています。
...
現在はこの API ファースト設計の上に、Discord コマンドと自然言語入力の両方から同じサーバー操作スキルを呼び出せるようにしています。ゲームサーバー管理は MCP の read-only tool と operation tool を分けて扱い、起動停止は確認付きで実行します。たとえば以下のような依頼に対応できます。
...
MCP 連携のハンドオフ資料は [docs/game-server-management-mcp-handoff.md](docs/game-server-management-mcp-handoff.md) に保存しています。

* [API\_CONTRACT.md](https://www.google.com/search?q=./API_CONTRACT.md) : 連携APIの入出力仕様を定義した「契約」ドキュメント。

### コントリビュータ向け設計メモ

スキル層への分離と、Cog との責務分担については [docs/architecture/skill-layer-refactor.md](docs/architecture/skill-layer-refactor.md) に整理しています。新しい運用系機能を追加する前に、この設計メモを参照してください。
依存方向とディレクトリ構成の図も同ファイルに含めています。
...
また、Discord へは何も送らずにゲームサーバー一覧キャッシュを扱いたい場合は、`/catalog/game-servers` を利用できます。`POST /catalog/game-servers` は Bot 内部の catalog repository に対して MCP の `list_servers` 再取得を指示し、`GET /catalog/game-servers` は再取得せず現在のキャッシュ内容だけを返します。

現在提供している Web API の仕様は [docs/web-api.md](docs/web-api.md) にまとめています。
...
* mock_api_server.py: 旧 REST 連携の動作確認用Flask Mockサーバー

## **🎮 ゲームサーバー管理機能**

ゲームサーバー管理用の MCP server と連携させることで、Discord からゲームサーバーを管理できます。接続先は `game_server_mcp_url` で設定し、既定値は `http://127.0.0.1:8000/mcp` です。

bot 起動時には MCP へ initialize し、list_tools / list_resources / list_resource_templates を実行して初期 discovery を行います。読み取り系 tool は自動実行し、起動停止は bot 側で確認フローを挟みます。

以下のコマンドが利用できます。

* /gs\_list: サーバー名、状態（オンライン/オフライン）、プレイヤー数、ゲーム内日数、リソース使用率の一覧を表示します。  
* /gs\_start \<サーバー名\>: 指定したサーバーの起動確認を開始します。  
